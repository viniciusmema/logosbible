...
export default function LogosApp() {
  ...
  useEffect(() => {
    ...
    // Carrega estudos do localStorage
    try {
      const saved = localStorage.getItem("logos_studies_v1");
  ...
  const persist = (arr) => {
    try {
      localStorage.setItem("logos_studies_v1", JSON.stringify(arr));
    } catch (e) {}
  };
  ...
  const generate = async () => {
    ...
    try {
      ...
      setError("Erro ao gerar o estudo. Verifique sua chave de API e tente novamente.");
    }
    setLoading(false);
  };
